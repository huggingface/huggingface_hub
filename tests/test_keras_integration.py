...
class HubKerasSequentialTest(HubMixingTestKeras):
    ...
    def test_push_to_hub_tensorboard(self):
        os.mkdir("{WORKING_REPO_DIR}/tb_log_dir")
        with open("{WORKING_REPO_DIR}/tb_log_dir/tensorboard.txt", "w") as fp:
            fp.write("Keras FTW")
        REPO_NAME = repo_name("PUSH_TO_HUB")
        model = self.model_init()
        model.build((None, 2))
        push_to_hub_keras(
            model,
            repo_path_or_name=f"{WORKING_REPO_DIR}/{REPO_NAME}",
            tensorboard_dir="{WORKING_REPO_DIR}/tb_log_dir",
            api_endpoint=ENDPOINT_STAGING,
            use_auth_token=self._token,
            git_user="ci",
            git_email="ci@dummy.com",
        ...
        self.assertEqual(
            os.path.exists(f"{WORKING_REPO_DIR}/{REPO_NAME}/logs/tensorboard.txt"), True
        )
