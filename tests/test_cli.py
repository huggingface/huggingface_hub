...
class TestGlobalOptions:
    def test_no_color_disables_typer_styling(self, runner: CliRunner, tmp_path: Path) -> None:
        local_file = tmp_path / "app.py"
        local_file.write_text("print('hello')\n")
        fake_api = SimpleNamespace(upload_file=Mock(return_value=SimpleNamespace(oid="commit-sha")))

        with patch("huggingface_hub.cli.spaces.get_hf_api", return_value=fake_api):
            colored_result = runner.invoke(
                app,
                [
                    "spaces",
                    "hot-reload",
        ...
        assert colored_result.exit_code == 0, colored_result.output
        assert "\u001b[" in colored_result.output
        assert result.exit_code == 0, result.output
        assert "\u001b[" not in result.output

    def test_no_color_env_var_disables_typer_styling(self, runner: CliRunner, tmp_path: Path) -> None:
        local_file = tmp_path / "app.py"
        local_file.write_text("print('hello')\n")
        fake_api = SimpleNamespace(upload_file=Mock(return_value=SimpleNamespace(oid="commit-sha")))
