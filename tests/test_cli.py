...
class TestJobsCommand:
    ...
    @pytest.mark.parametrize(
        ("initiator_payload", "expected_type", "expected_id", "expected_name"),
        [
            (
                {"type": "user", "id": "5e9ecfc04957053f60648a3e", "name": "lhoestq"},
                "user",
                "5e9ecfc04957053f60648a3e",
                "lhoestq",
            ),
            (
                {"type": "org", "id": "60107b385ac3e86b3ea4fc34", "name": "huggingface"},
                "org",
                "60107b385ac3e86b3ea4fc34",
                "huggingface",
            ),
            (
                {"type": "scheduled-job", "id": "69bbd2df71691dc46f160d3b"},
                "scheduled-job",
                "69bbd2df71691dc46f160d3b",
                None,
            ),
            (
                {"type": "duplicated-job", "id": "69aaa1d871691dc46f160abc"},
                "duplicated-job",
                "69aaa1d871691dc46f160abc",
                None,
            ),
        ],
    )
    def test_job_info_parses_initiator_variants(
        self,
        ...
        """`JobInfo` parses each `initiator` variant returned by the Hub (user / org / scheduled-job / duplicated-job)."""
        from huggingface_hub._jobs_api import JobInfo
        ...
        assert job.initiator is not None
        assert job.initiator.type == expected_type
        assert job.initiator.id == expected_id
        assert job.initiator.name == expected_name

    def test_job_info_initiator_default_none(self) -> None:
        """`JobInfo` parses cleanly when the Hub response omits `initiator` (older payloads, edge cases)."""
        from huggingface_hub._jobs_api import JobInfo
