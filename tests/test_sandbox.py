...
class TestPoolLifecycle:
    ...
    def test_a_failed_cancellation_raises_and_keeps_the_host_discoverable(self, fake_server: str, monkeypatch) -> None:
        ...
        pool.close()  # __exit__ may repeat close after the first call raised.
    ...
    def test_close_drain_allows_the_configured_boot_timeout(self, fake_server, monkeypatch):
        pool = self._pool(fake_server, monkeypatch)
        pool._in_flight = 1
        wait = MagicMock(return_value=True)
        monkeypatch.setattr(pool._drained, "wait_for", wait)
        pool.close()
        assert wait.call_args.kwargs["timeout"] > pool._start_timeout

    def test_warm_only_boots_remaining_room_with_unadopted_hosts(self, fake_server, monkeypatch):
        foreign = _pool_host_job("foreign")
        foreign.initiator.id = "someone-else"
        pool = self._pool(fake_server, monkeypatch, jobs=[foreign], max_hosts=3, warm_up=3)
        assert pool.num_hosts == 2
        pool._hosts[1].job_id = "second"  # the boot stub otherwise reuses job123
        pool.warm(3)
        assert pool.num_hosts == 2

    def test_listing_hosts_does_not_hold_the_pool_lock(self, fake_server, monkeypatch):
        pool = self._pool(fake_server, monkeypatch)
        acquired = []
        ...
        monkeypatch.setattr(pool._api, "list_jobs", list_jobs)
        pool.max_hosts = 1
        with pytest.raises(SandboxError, match="max_hosts"):
            pool._provision_hosts(1)
...
class TestHostAdmission:
    ...
    @pytest.mark.parametrize("field", ["command", "flavor"])
    def test_a_missing_spec_is_refused(self, field) -> None:
        job = _pool_host_job()
        setattr(job, field, None)
        assert self._reject(job, policy=sandbox_mod.ADOPT_NAMESPACE) is not None
...
class TestPoolCacheFile:
    ...
    def test_the_cache_is_private_to_this_user(self) -> None:
        ...
        assert {p.name for p in path.parent.iterdir()} <= {"p.json", "p.json.lock"}
...
class TestPoolCacheBinding:
    ...
    def test_delete_treats_pool_names_literally(self) -> None:
        for name in ("p*", "p1", "p[1]"):
            _save_cache(name, [_cached_host()])
        sandbox_mod.delete_pool_cache("p*")
        assert read_pool_cache("p*", _ctx()) is None
        assert read_pool_cache("p1", _ctx()) is not None
        assert read_pool_cache("p[1]", _ctx()) is not None
...
class TestCachedHostFreshness:
    ...
    def test_an_unreachable_jobs_api_preserves_the_entry(self, monkeypatch) -> None:
        ...
        pool = self._pool()
        with pytest.raises(SandboxError, match="retry later"):
            pool._seed_hosts_from_cache()
        assert pool._hosts == []
        assert pool._dead_host_ids == set()

    def test_identity_failure_preserves_cache_and_can_be_retried(self, monkeypatch):
        _save_cache("p1", [_cached_host("hostA", age=HOST_TRUST_TTL + 60)])
        monkeypatch.setattr(
            sandbox_mod.HfApi, "whoami", MagicMock(side_effect=[OSError("offline"), {"id": PRINCIPAL_ID}])
        )
        monkeypatch.setattr(
            sandbox_mod.HfApi, "inspect_job", MagicMock(return_value=_pool_host_job("hostA", pool_name="p1"))
        )
        pool = self._pool()
        with pytest.raises(SandboxError, match="current user"):
            pool.create()
        assert read_pool_cache("p1", pool._cache_context) is not None
        assert pool._dead_host_ids == set()
        pool._seed_hosts_from_cache()
        assert pool.host_ids == ["hostA"]
    ...
    def test_missing_job_is_pruned(self, monkeypatch):
        ...
        pool = self._pool()
        pool._seed_hosts_from_cache()
        assert pool._dead_host_ids == {"hostA"}
