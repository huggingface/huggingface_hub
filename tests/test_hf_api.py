...
class HfApiEndpointsTest(HfApiCommonTest):
    ...
    def tearDownClass(cls):
        for FILE_KEY, FILE_PATH in FILES:
            cls._api.delete_obj(
                token=cls._token, filetype="datasets", filename=FILE_KEY
            )
    ...
    def test_presign(self):
        for FILE_KEY, FILE_PATH in FILES:
            urls = self._api.presign(
                token=self._token, filetype="datasets", filename=FILE_KEY
            )
            self.assertIsInstance(urls, PresignedUrl)
...
class HfLargefilesTest(HfApiCommonTest):
    ...
    def test_end_to_end_thresh_6M(self):
        ...
        self.assertIn(
            "transformers-cli lfs-enable-largefiles", failed_process.stderr.decode()
        )
