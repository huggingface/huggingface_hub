...
class KerasModelHubMixin(PushToHubMixin):
    def __init__(self, *args, **kwargs):
        ...
        Example::
            ...
            >>> model = MyModel()
            >>> model.save_pretrained("mymodel", config=self.get_config(), push_to_hub=False)
            >>> model.push_to_hub("mymodel", "model-1")

            >>> # Save full model and push to Hub, useful if you need to restore custom objects.
            >>> model = MyModel()
            >>> model.save_pretrained("mymodel", push_to_hub=False, saved_model=True)
            >>> model.push_to_hub("mymodel")
        ...
        **kwargs,
        ...
        if saved_model:
            saved_model_dir = os.path.join(
                save_directory, TF_SAVED_MODEL_NAME, str(version)
            )
    ...
    def from_pretrained(
        ...
        **model_kwargs,
        ...
        # This is required for models with custom classes.
        if saved_model:
            if os.path.isdir(model_id) and TF_SAVED_MODEL_NAME in os.listdir(model_id):
                saved_model_dir = os.path.join(
                    model_id, TF_SAVED_MODEL_NAME, str(version)
                )
                ...
                try:
                    ...
                    raise err(
                        f"Directory {TF_SAVED_MODEL_NAME} not found in Hugging Face Hub"
                    )
...
class SklearnPipelineHubMixin(PushToHubMixin):
    def __init__(self, *args, **kwargs):
        ...
        Example::
            >>> from huggingface_hub import SklearnPipelineHubMixin
            >>> class MyPipeline(Pipeline, SklearnPipelineHubMixin):
    ...
    def from_pretrained(
        ...
        """
        Instantiate a pre-trained sklearn pipeline from the Hugging Face Hub.

        Args:
            repo_id_or_path: (:obj:`str` or :obj:`os.PathLike`):
                Can be either:
                    ...
                    - You can add `revision` by appending `@` at the end of model_id simply like this
                        ``dbmdz/bert-base-german-cased@main``. A revision is the specific model version to use.
            ...
            use_auth_token (:obj:`str` or `bool`, `optional`):
                ...
                the token generated when running :obj:`huggingface-cli login` (stored in :obj:`~/.huggingface`).
            ...
            proxies (:obj:`Dict[str, str], `optional`):
                ...
                'http://hostname': 'foo.bar:4012'}`. The proxies are used on each request.
        """
        model_id = repo_id_or_path
        revision = None
        if len(model_id.split("@")) == 2:
            model_id, revision = model_id.split("@")

        if os.path.isdir(model_id) and model_filename in os.listdir(model_id):
