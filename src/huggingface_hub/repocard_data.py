...
def eval_result_entries_to_yaml(entries: list[EvalResultEntry]) -> list[dict[str, Any]]:
    ...
    Example:
        ```python
        >>> from huggingface_hub.repocard_data import EvalResultEntry, eval_result_entries_to_yaml
        >>> entries = [
...
def yaml_to_eval_result_entries(data: list[dict[str, Any]]) -> list[EvalResultEntry]:
    """Parse a list of dicts (from `.eval_results/*.yaml`) into [`EvalResultEntry`] objects.

    Args:
        data (`list[dict[str, Any]]`):
            A list of dictionaries parsed from a YAML file.
    ...
    Example:
        ```python
        >>> from huggingface_hub.repocard_data import yaml_to_eval_result_entries
        >>> yaml_data = [
        ...     {"dataset": {"id": "cais/hle", "task_id": "default"}, "value": 20.90},
        ...     {"dataset": {"id": "Idavidrein/gpqa", "task_id": "gpqa_diamond"}, "value": 0.412},
        ... ]
        >>> entries = yaml_to_eval_result_entries(yaml_data)
