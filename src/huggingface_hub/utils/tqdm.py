...
def hf_thread_map(
    ...
    *,
    ...
    tqdm_class: type[old_tqdm] | None = None,
    cancel_on_interrupt: Callable[[], None] | None = None,
    **tqdm_kwargs,
) -> list[R]:
    ...
    Args:
        ...
        cancel_on_interrupt (`Callable`, *optional*):
            Callback invoked on `KeyboardInterrupt` after pending futures are cancelled. Use it to cooperatively stop
            tasks that are already running before the executor waits for them to finish.
        **tqdm_kwargs:
    ...
    with (
        ...
        try:
            ...
            for future in future_to_index:
                future.cancel()
            if isinstance(error, KeyboardInterrupt) and cancel_on_interrupt is not None:
                cancel_on_interrupt()
            raise
