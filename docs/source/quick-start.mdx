...
<Tip>
It is also possible to login programmatically without asking the user for the token by directly
passing the token to [`login`] (i.e. `login(token="hf_xxx")`). If you do so, be careful when
sharing your source code. It is a best practice to load the token from a secure vault instead
of saving it in plain in your codebase/notebook.
</Tip>

<Tip>
Another way to authenticate is to provide your token when calling every methods. This way your token
is never stored on the machine.
</Tip>
...
To upload more than one file at a time, take a look at this [guide](./guides/upload)
