...
If you are unfamiliar with environment variable, here are generic articles about how to manage them
[on MacOS and Linux](https://linuxize.com/post/how-to-set-and-list-environment-variables-in-linux/)
and on [Windows machines](https://phoenixnap.com/kb/windows-set-environment-variable).
...
Configure the 🤗 Hub base url. You might want to set this variable if your organization
...
Configure when `huggingface_hub` will store data. In particular the cache will be stored
in this folder.
...
Configure where repositories from the Hub will be cached locally (models, datasets and
spaces).
...
Configure where [assets](guides/how-to-cache#caching-assets) created by downstream libraries
...
Configure the User Access Token to authenticate to the Hub. If set, this value will
...
You can disable all the progress bars at once by settings `HF_HUB_DISABLE_PROGRESS_BARS=1`.
...
Boolean value. If set to `1`, `huggingface_hub` will interact with the staging environment.
