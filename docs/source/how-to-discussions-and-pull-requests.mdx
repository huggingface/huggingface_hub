...
>>> get_repo_discussions(repo_id="bigscience/bloom-1b3")
Pagination(
    page=[
        Discussion(
            num=8,
...
`HfApi.get_repo_discussions` returns a [`Pagination`] object, which is a convenience wrapper to manage pagination
...
Opening a Pull Requests this way can be useful if you need to work on changes locally. Pull Requests opened this way will be in `"draft"` mode.
