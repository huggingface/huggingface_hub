...
one. It is a Space to launch ephemeral environments when a PR is opened on a Space.
...
Good job! You just created a webhook server. But what happened exactly? Let's break it down:
