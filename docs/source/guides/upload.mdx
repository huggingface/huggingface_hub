...
The Hugging Face Hub makes it easy to save and version data. However, it has some limitations, especially when it comes
to updating thousands of times the same file. For instance, you might want to save logs of a training process or user
feedback on a deployed Space. In those cases, it makes sense to upload the data as a `dataset` on the Hub but it can be
hard to do it properly. The main reason for that is that you don't want to version every single update of your data -
which would make the git repository unusable. The [`CommitScheduler`] class offers an interface to solve this problem.

The idea is to run a background job that will regularly push a local folder to the Hub. Let's assume that you have a
Gradio Space that takes as input a text and generates two translations of it. Then, the user can select its preferred
translation. For each run, you want to save the input, output and user preference to analyze the results. This is a
perfect use case for [`CommitScheduler`]: you want to save data to the Hub (potentially millions of user feedback) but
you don't _need_ to save on real time at each user input. Instead, you can save the data locally in a JSON file and
upload it only every 10 minutes. Here is an example on how to do that:
