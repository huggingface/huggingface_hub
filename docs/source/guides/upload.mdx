...
restarts which often implies generating uuids. Beside robustness, you would ideally like to upload data in a format that
is readable by the `datasets` library for later reuse. We compiled in this [interactive demo Space](https://huggingface.co/spaces/Wauplin/demo_commit_scheduler)
implementations to tackle major use cases. Depending on your specific needs, you might need to adapt them

### Custom uploads

As said above, [`CommitScheduler`] assumes that your data is append-only and should be uploading "as is". However, you
might want to customize the way data is uploaded. You can do that by creating a class inheriting from [`CommitScheduler`]
and overwrite the `push_to_hub` method. You can overwrite this method the way you want. You are guaranteed that it will
be called every `every` minutes in a background thread. You don't have to worry about concurrency and errors but you
must be careful other aspects like not pushing empty commits or duplicated data.
...
You can find a complete code in [this Gradio app](https://huggingface.co/spaces/Wauplin/demo_commit_scheduler/blob/main/app_1M_image.py).
...
- Repo parameters: `repo_id`, `repo_type`, `revision`, `token`
