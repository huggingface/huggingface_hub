...
>>> repo_id = "Wauplin/dreambooth-training"
>>> api = HfApi()
...
However, you might want to configure it further with secret and upgraded hardware.
...
For example, an HF token to upload your an image dataset to the Hub once generated from your Space.
...
Huggingface Spaces allow for a lot of different use cases. In some cases you might want
to temporarily run a Space on a specific hardware, do something and then shut it down.

In this guide, we will explore how to benefit from HF Spaces to finetune a model on demand.
This is only one way of solving a particular problem. It has to be taken as a suggestion
and adapted to your own use case.

Let's assume we have a Space to finetune a model. It is a Gradio app that takes as input
a model id and a dataset id. The workflow is as follow:
...
Step 3. requires a custom hardware but you don't want it to be running all time on a paid
GPU. A possible solution is to dynamically request a hardware for the training and shut it
...
run it on the correct hardware. Once done, set back hardware to a the free-plan cpu and
...
# On Space startup, check if a task is schedule. If yes, finetune the model. If not,
...
<Tip warning={true}>
**Disclaimer:** such a workflow does not support concurrent access as normal demos.
...
def get_task():
    csv_file = 
    with open(
        hf_hub_download(repo_id=TASK_DATASET_ID, filename="tasks.csv", repo_type="dataset" token=HF_TOKEN)
    ) as csv_file:
...
def mark_as_done(task):
    ...
    api.upload_file(
        ...
        path_or_fileobj=tasks_text.replace(
