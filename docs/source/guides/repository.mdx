...
(`git push`). This is still valid when using the Hugging Face Hub. However, software engineering and machine learning do
not share the same requirements and workflows. In ML, it may be more efficient to use HTTP, so you don't have to maintain a local folder with massive model weights. You can read our [Git vs HTTP paradigm](../concepts/git_vs_http) explanation page
for more details.
