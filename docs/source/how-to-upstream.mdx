...
Sharing your files and work is a very important aspect of the Hub. The `huggingface_hub` offers several options for uploading your files to the Hub. You can use these functions independently or integrate them into your library, making it more convenient for your users to interact with the Hub. This guide will show you how to:

* Upload a file with [`upload_file`].
* Upload a folder with [`upload_folder`].
...
* Use [`create_commit`] to push files without Git.
...
Once you've created a repository with the [`create_repo`](./how-to-manage#create-a-repository) function, you can upload your files to your repository. Use the [`upload_file`] function to upload a file. 
...
...                   repo_type="dataset",
... )
# https://huggingface.co/datasets/username/test-dataset/tree/main/my-dataset/train/
```
...
The [`Repository`] class has a [`~Repository.push_to_hub`] function to add files, make a commit, and push them to a repository. Unlike the `commit` context manager, requires you'll need to pull from a repository first before calling [`~Repository.push_to_hub`].
...
Once you're ready, push the file to your repository with [`~Repository.git_push`]:
...
## Upload without Git installed

<Tip>

The [`create_commit`] function is a low-level API for working at a commit level. The [`upload_file`] and [`upload_folder`] functions are higher-level APIs that use [`create_commit`] under the hood. We recommend trying these functions first if you don't need to work at a lower level.

</Tip>

If you don't have Git installed on your system, use the [`create_commit`] function to upload your file. [`create_commit`] uses the HTTP protocol to upload files to the Hub. Large and binary files are automatically uploaded with the Git LFS protocol.

There are two kinds of operations supported by the [`create_commit`] function:
...
[`create_commit`] enables each of these other functions:

* [`upload_file`] uploads a single file to a repository on the Hub.
* [`upload_folder`] uploads a local directory to a repository on the Hub.
* [`delete_file`] deletes a single file from a repository on the Hub. 
* [`metadata_update`] updates a repository's metadata.

For more detailed information about these functions, take a look at the [`HfApi`] reference.
