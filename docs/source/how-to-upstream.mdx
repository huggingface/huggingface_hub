...
* Push files with the `commit` context manager.
...
Once you've created a repository with the [`create_repo`](./how-to-manage#create-a-repository) function, you can upload files to your repository with the [`upload_file`] function. 
...
For large files (>5GB), you need to install a custom transfer agent for Git LFS:

```bash
huggingface-cli lfs-enable-largefiles
```

You should install this for each model repository that contains a model file. Once installed, you'll be able to push files larger than 5GB.
...
If you don't have Git installed on your system, use the [`create_commit`] function to upload your file. [`create_commit`] uses the HTTP protocol to upload files to the Hub. Large and binary files are automatically uploaded with Git LFS.
