...
* with with the `commit` context manager.
...
Files larger than 10MB are automatically handled by Git LFS. But for very large files (>5GB), you need to install a custom transfer agent for Git LFS:
