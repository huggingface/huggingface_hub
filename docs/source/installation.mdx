...
Now you’re ready to install 🤗 `huggingface_hub` with the following command:
...
Once installed, check if 🤗 `huggingface_hub` is properly running with the following command:
...
If you are more familiar with it, you can install 🤗 `huggingface_hub` using [conda-forge channel](https://anaconda.org/conda-forge/huggingface_hub):
