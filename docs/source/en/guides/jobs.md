...
`hf jobs uv run ocr.py` then launches with the right image, hardware and interpreter, and `--flavor`, `-e`, ... still override what the script declares. When the script is a URL, `secrets`, `volumes`, `namespace` and `network_group` must be passed as flags instead. See the [CLI guide](./cli#ship-the-launch-config-with-the-script) for the full list of keys and the merge rules.
