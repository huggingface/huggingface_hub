...
`hf jobs uv run ocr.py` then launches with the right image, hardware and interpreter, and `--flavor`, `-e`, ... still override what the script declares. If the table requests `secrets`, `volumes`, `namespace` or `network_group`, the CLI asks for confirmation first, unless you pass the matching flags. See the [CLI guide](./cli#ship-the-launch-config-with-the-script) for the full list of keys and the merge rules.
