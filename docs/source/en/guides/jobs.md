...
Mount a volume on the Jobs's disk using a list of [`JobVolume`].

You can mount any Hugging Face Repository (model/dataset/space) or [Storage Bucket](/docs/hub/storage-buckets). For example:

* mount a model repository: `JobVolume(type="model", source="openai/gpt-oss-120b", mount_path="/model")`
* mount a dataset repository: `JobVolume(type="dataset", source="HuggingFaceFW/fineweb", mount_path="/data")`
* mount a storabe bucket: `JobVolume(type="bucket", source="username/my-bucket", mount_path="/mnt)`
...
...     volumes=[JobVolume(type="dataset", source="HuggingFaceFW/fineweb", mount_path="/data")],
... )
```
