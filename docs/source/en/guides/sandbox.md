...
A command that exits non-zero raises [`SandboxCommandError`] (with `stdout`, `stderr` and `exit_code` attached). Pass `check=False` to get the [`SandboxCommandResult`] back instead of raising:
