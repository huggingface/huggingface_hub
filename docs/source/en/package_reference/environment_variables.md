...
It also checks (at most once every 24 hours, purely locally) whether the `hf-cli` skill is installed and generated by the running version, and suggests `hf skills add` / `hf skills update` if not. Nothing is ever installed or updated automatically.
